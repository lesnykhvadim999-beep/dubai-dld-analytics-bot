...
# =========================
# v69 LUXURY ADAPTIVE FLOW OVERRIDE
# =========================
# Цель: один чистый сценарий без старого перегруженного меню.
# Главное меню короткое; внутри процесса показываются только релевантные кнопки.

LAST_REPORTS = {}
LAST_REPORT_META = {}


def t69(user_id, ru, en=None, ar=None):
    l = lang(user_id)
    if l == "en":
        return en or ru
    if l == "ar":
        return ar or (en or ru)
    return ru
...
def main_menu(user_id):
    return menu69([
        [t69(user_id, "🧠 Подбор", "🧠 Selection", "🧠 الاختيار"), t69(user_id, "🏢 Здание", "🏢 Building", "🏢 المبنى")],
        [t69(user_id, "🏙 Район", "🏙 Area", "🏙 المنطقة"), t69(user_id, "📊 Рейтинги", "📊 Ratings", "📊 التصنيفات")],
        [t69(user_id, "🧾 Сделки", "🧾 Deals", "🧾 الصفقات"), t69(user_id, "🌆 Дубай", "🌆 Dubai", "🌆 دبي")],
    ])


def nav_menu(user_id):
    return menu69([[tr(user_id, "back"), tr(user_id, "main")]])


def result_menu(user_id):
    return menu69([
        [t69(user_id, "📄 PDF", "📄 PDF", "📄 PDF"), t69(user_id, "💼 Заявка", "💼 Request", "💼 طلب")],
        [t69(user_id, "🔁 Изменить", "🔁 Change", "🔁 تعديل"), tr(user_id, "main")],
    ])


def deal_menu69(user_id):
    return menu69([
        [tr(user_id, "sale"), tr(user_id, "rent")],
        [tr(user_id, "both")],
        [tr(user_id, "back"), tr(user_id, "main")],
...
def report_menu69(user_id):
    return menu69([
        [t69(user_id, "📊 Аналитика", "📊 Analytics", "📊 التحليل"), t69(user_id, "🧾 Сделки", "🧾 Deals", "🧾 الصفقات")],
        [t69(user_id, "📈 Периоды", "📈 Periods", "📈 الفترات"), t69(user_id, "💼 Резюме", "💼 Summary", "💼 ملخص")],
        [tr(user_id, "back"), tr(user_id, "main")],
    ])


def deal_scope_menu69(user_id):
    return menu69([
        [t69(user_id, "🏢 По зданию", "🏢 By building", "🏢 حسب المبنى"), t69(user_id, "🏙 По району", "🏙 By area", "🏙 حسب المنطقة")],
        [t69(user_id, "🌆 По Дубаю", "🌆 Dubai-wide", "🌆 دبي")],
        [tr(user_id, "back"), tr(user_id, "main")],
    ])


def ratings_menu69(user_id):
    return menu69([
        [t69(user_id, "🏢 Активные", "🏢 Active", "🏢 نشطة"), t69(user_id, "💰 Цены", "💰 Prices", "💰 الأسعار")],
        [t69(user_id, "🏙 Районы", "🏙 Areas", "🏙 المناطق"), t69(user_id, "📈 Рост", "📈 Growth", "📈 النمو")],
        [tr(user_id, "back"), tr(user_id, "main")],
    ])


def deal_type_from_button(user_id, text):
    if text == tr(user_id, "sale"):
        return "sale"
    if text == tr(user_id, "rent"):
        return "rent"
    if text == tr(user_id, "both"):
        return None
    return None


def period_from_button(user_id, text):
    mapping = {
        tr(user_id, "p3"): "3",
        tr(user_id, "p6"): "6",
        tr(user_id, "p12"): "12",
...
def save_report69(user_id, text, meta=None):
    LAST_REPORTS[user_id] = text
    LAST_REPORT_META[user_id] = meta or {}


def loading_text69(user_id):
    return t69(
        user_id,
        "⏳ <b>Идёт обработка DLD-данных</b>\n\n◇ Подключаю архив, live-базу и intelligence-слой.\n◇ Проверяю фильтры и ликвидность.\n◇ Формирую профессиональное заключение.\n\nПожалуйста, подождите — отчёт готовится.",
        "⏳ <b>Processing DLD data</b>\n\n◇ Connecting archive, live database and intelligence layer.\n◇ Checking filters and liquidity.\n◇ Preparing professional conclusion.\n\nPlease wait — the report is being prepared.",
        "⏳ <b>جاري معالجة بيانات DLD</b>\n\nيرجى الانتظار — يتم إعداد التقرير.",
    )


def conclusion360_69(row=None, scope=None, name=None, prop=None, period=None, deal_type=None, rows=None):
    deals = _int(row.get("deals")) if row else (len(rows) if rows else 0)
    avg_price = row.get("avg_price") if row else None
    avg_meter = row.get("avg_meter") if row else None
    rent = is_rent_deal_type(deal_type)
    scope_name = name or "Dubai"
    if deals <= 0:
        return (
            "\n\n🧠 <b>Экономическое заключение 360°</b>\n\n"
            "По выбранному фильтру данных недостаточно для уверенного вывода. "
            "Рекомендуется расширить период, убрать комнатность или выбрать более крупный рынок — район вместо здания."
        )
    deal_word = "аренды" if rent else "продаж"
    price_label = "средняя годовая аренда" if rent else "средняя цена сделки"
    return (
        "\n\n🧠 <b>Экономическое заключение 360°</b>\n\n"
        f"Рынок <b>{scope_name}</b> показывает <b>{format_int(deals)}</b> DLD-сделок по выбранной конфигурации. "
        f"Это основной индикатор ликвидности: чем больше сделок, тем легче понять реальную рыночную цену и быстрее выйти из позиции.\n\n"
        f"💰 {price_label.capitalize()}: <b>{format_money(avg_price)}</b>.\n"
        f"📐 Средняя цена за метр: <b>{format_money(avg_meter)}</b>.\n\n"
        f"Для инвестора это означает: сравнивать конкретный объект нужно не с рекламными ценами, а с последними DLD-сделками {deal_word}. "
        "Если цена входа ниже среднего рынка при нормальной ликвидности — объект можно рассматривать в первую очередь."
    )


def no_data69(user_id, title="Аналитика"):
    return t69(
        user_id,
        f"⚠️ <b>{title}</b>\n\nПо выбранным фильтрам не удалось получить стабильную выборку DLD.\n\nЧто сделать:\n• выбрать больший период;\n• убрать комнатность;\n• проверить район вместо здания;\n• выбрать другой тип сделки.",
        f"⚠️ <b>{title}</b>\n\nNo stable DLD sample was found for the selected filters. Try a longer period, remove bedroom filter, or analyze area instead of building.",
        f"⚠️ <b>{title}</b>\n\nلا توجد عينة كافية حسب الفلاتر المختارة.",
    )


def format_deal_rows69(rows, title, user_id):
    out = f"🧾 <b>{title}</b>\n\n"
    for i, r in enumerate(rows[:10], 1):
        out += (
            f"<b>{i}.</b> {r.get('safe_date') or '-'}\n"
            f"🏢 {r.get('building_name_en') or '-'}\n"
            f"📍 {r.get('area_name_en') or '-'}\n"
            f"🏠 {r.get('rooms_en') or '-'} / {r.get('property_sub_type_en') or r.get('property_type_en') or '-'}\n"
...
def generate_pdf69(user_id, text):
    try:
        try:
        ...
        fd, path = tempfile.mkstemp(prefix="dld_report_", suffix=".pdf")
        os.close(fd)
        doc = SimpleDocTemplate(path, pagesize=A4, rightMargin=36, leftMargin=36, topMargin=36, bottomMargin=36)
        styles = getSampleStyleSheet()
        normal = ParagraphStyle("NormalUnicode", parent=styles["Normal"], fontName=font_name, fontSize=10.5, leading=15)
        title = ParagraphStyle("TitleUnicode", parent=styles["Title"], fontName=font_name, fontSize=16, leading=20, spaceAfter=14)
        story = [Paragraph("Dubai DLD Intelligence Report", title)]
        plain = html_to_plain69(text).replace("\n", "<br/>")
        story.append(Paragraph(plain, normal))
        story.append(Spacer(1, 12))
        doc.build(story)
        return path
    except Exception as e:
        print("PDF69_ERROR", repr(e))
        return None


def compact_main_text69(user_id):
    return t69(user_id, "🏠 <b>Главное меню</b>\n\nВыберите раздел:", "🏠 <b>Main menu</b>\n\nChoose section:", "🏠 <b>القائمة الرئيسية</b>")


@dp.message(Command("language"))
async def language_command69(message: Message):
    user_states[message.from_user.id] = {}
    await message.answer(TEXTS["ru"]["choose_lang"], reply_markup=language_menu())


@dp.message(Command("pdf"))
async def pdf_command69(message: Message):
    await send_pdf69(message)


@dp.message(Command("admin"))
async def admin_command69(message: Message):
    user_id = message.from_user.id
    admin_ids = [x.strip() for x in os.getenv("ADMIN_IDS", "").split(",") if x.strip()]
    if str(user_id) not in admin_ids:
        await message.answer(f"⚠️ ADMIN_IDS не задан или ваш ID не добавлен.\n\nВаш Telegram ID: <code>{user_id}</code>")
        return
    await message.answer("👑 <b>Админ-панель</b>\n\nБот онлайн. Расширенная статистика будет подтягиваться из bot_stats таблиц.", reply_markup=main_menu(user_id))


async def send_pdf69(message: Message):
    user_id = message.from_user.id
    report = LAST_REPORTS.get(user_id)
    if not report:
        await message.answer(t69(user_id, "⚠️ Сначала сформируйте отчёт, затем нажмите PDF.", "⚠️ Generate a report first, then press PDF."), reply_markup=main_menu(user_id))
        return
    await message.answer(t69(user_id, "📄 Формирую PDF-отчёт…", "📄 Preparing PDF report…"))
    path = generate_pdf69(user_id, report)
    if not path:
        await message.answer(t69(user_id, "⚠️ PDF временно недоступен. Отчёт можно скопировать из сообщения выше.", "⚠️ PDF is temporarily unavailable."), reply_markup=result_menu(user_id))
        return
    await message.answer_document(FSInputFile(path, filename="Dubai_DLD_Report.pdf"), caption=t69(user_id, "📄 Готово. PDF-отчёт сформирован.", "📄 Done. PDF report is ready."), reply_markup=result_menu(user_id))


async def run_report69(message: Message, report_type):
    user_id = message.from_user.id
    st = user_states.get(user_id, {})
    scope = st.get("scope", "dubai")
    name = st.get("name")
    prop = st.get("property")
    period = st.get("period")
    deal_type = st.get("deal_type")
    await message.answer(loading_text69(user_id))

    if report_type == "deals":
        rows, up, uper, udt = get_latest_deals_smart(scope, name, prop, period, deal_type, limit=10)
        if not rows:
            await message.answer(no_data69(user_id, "Сделки DLD"), reply_markup=result_menu(user_id))
            return
        title = name or "Dubai"
        response = format_deal_rows69(rows, f"Сделки DLD — {title}", user_id)
        row, _, _, _ = get_stats_smart(scope, name, up, uper, udt)
        response += conclusion360_69(row, scope, name, up, uper, udt, rows)
        save_report69(user_id, response, st)
        await message.answer(response, reply_markup=result_menu(user_id))
        return

    if report_type == "periods":
        comp = get_comparison(scope, name, prop, period or "6", deal_type)
        if not comp:
            await message.answer(no_data69(user_id, "Сравнение периодов"), reply_markup=result_menu(user_id))
            return
        current, previous = comp
        title = "📈 <b>Профессиональное сравнение периодов</b>"
        if name:
            title += f"\n{name}"
        response = show_comparison(title, current, previous, period or "6", deal_type)
        response += conclusion360_69(current, scope, name, prop, period, deal_type)
        save_report69(user_id, response, st)
        await message.answer(response, reply_markup=result_menu(user_id))
        return

    if report_type == "summary":
        row, up, uper, udt = get_stats_smart(scope, name, prop, period, deal_type)
        if not row:
            await message.answer(no_data69(user_id, "Экономическое резюме"), reply_markup=result_menu(user_id))
            return
        title = f"💼 <b>Экономическое резюме — {name or 'Dubai'}</b>"
        response = show_stats(title, row, up, uper, udt) + conclusion360_69(row, scope, name, up, uper, udt)
        save_report69(user_id, response, st)
        await message.answer(response, reply_markup=result_menu(user_id))
        return
    ...
    row, up, uper, udt = get_stats_smart(scope, name, prop, period, deal_type)
    if not row:
        await message.answer(no_data69(user_id, "Аналитика DLD"), reply_markup=result_menu(user_id))
        return
    title = f"📊 <b>Аналитика DLD — {name or 'Dubai'}</b>"
    response = show_stats(title, row, up, uper, udt) + conclusion360_69(row, scope, name, up, uper, udt)
    save_report69(user_id, response, st)
    await message.answer(response, reply_markup=result_menu(user_id))


async def ask_next_filter69(message: Message):
    user_id = message.from_user.id
    st = user_states.get(user_id, {})
    step = st.get("step")
    if step == "choose_deal_type69":
        await message.answer(tr(user_id, "choose_deal_type"), reply_markup=deal_menu69(user_id))
    elif step == "choose_property69":
        await message.answer(tr(user_id, "choose_property"), reply_markup=property_menu69(user_id))
    elif step == "choose_period69":
        await message.answer(tr(user_id, "choose_period"), reply_markup=period_menu69(user_id))
    elif step == "choose_report69":
        await message.answer(t69(user_id, "📊 <b>Что показать?</b>\n\nВыберите тип отчёта.", "📊 <b>What to show?</b>"), reply_markup=report_menu69(user_id))
    else:
        await message.answer(compact_main_text69(user_id), reply_markup=main_menu(user_id))
...
async def main_handler(message: Message):
    ...
    try:
        ...
        if text == tr(user_id, "back"):
            ...
            if hist:
                user_states[user_id] = hist.pop()
                user_states[user_id]["history"] = hist
                await ask_next_filter69(message)
            else:
                user_states[user_id] = {}
                await message.answer(compact_main_text69(user_id), reply_markup=main_menu(user_id))
            return
        if text in ["📄 PDF", "PDF"]:
            await send_pdf69(message)
            return
        if text in ["💼 Заявка", "💼 Request"]:
            await message.answer("💼 Для консультации и заявки перейдите в бот: @dubai_fpr_lead_bot", reply_markup=result_menu(user_id))
            return
        if text in ["🔁 Изменить", "🔁 Change"]:
            # возвращаемся к началу выбранного сценария
            scope = st.get("scope", "dubai")
            if scope == "building":
                user_states[user_id] = {"step": "building_query69", "scope": "building"}
                await message.answer(tr(user_id, "enter_building"), reply_markup=nav_menu(user_id))
            elif scope == "area":
                user_states[user_id] = {"step": "area_query69", "scope": "area"}
                await message.answer(tr(user_id, "enter_area"), reply_markup=nav_menu(user_id))
            else:
                user_states[user_id] = {"step": "choose_deal_type69", "scope": "dubai"}
                await ask_next_filter69(message)
            return

        # Main menu
        if text in ["🧠 Подбор", "🧠 Selection"]:
            user_states[user_id] = {"step": "smart_goal69"}
            await message.answer("🧠 <b>Инвестиционный подбор</b>\n\nВыберите цель покупки:", reply_markup=menu69([["💰 Доходность", "🏡 Для жизни"], ["📈 Перепродажа", "🔑 Аренда"], [tr(user_id, "main")]]))
            return
        if text in ["🏢 Здание", "🏢 Building"]:
            user_states[user_id] = {"step": "building_query69", "scope": "building"}
            await message.answer(tr(user_id, "enter_building"), reply_markup=nav_menu(user_id))
            return
        if text in ["🏙 Район", "🏙 Area"]:
            user_states[user_id] = {"step": "area_query69", "scope": "area"}
            await message.answer(tr(user_id, "enter_area"), reply_markup=nav_menu(user_id))
            return
        if text in ["🌆 Дубай", "🌆 Dubai"]:
            user_states[user_id] = {"step": "choose_deal_type69", "scope": "dubai", "history": []}
            await ask_next_filter69(message)
            return
        if text in ["🧾 Сделки", "🧾 Deals"]:
            user_states[user_id] = {"step": "deals_scope69"}
            await message.answer("🧾 <b>Сделки DLD</b>\n\nГде показать сделки?", reply_markup=deal_scope_menu69(user_id))
            return
        if text in ["📊 Рейтинги", "📊 Ratings"]:
            user_states[user_id] = {"step": "ratings69"}
            await message.answer("📊 <b>Рейтинги</b>\n\nВыберите рейтинг:", reply_markup=ratings_menu69(user_id))
            return

        # Ratings
        if st.get("step") == "ratings69":
            await message.answer(loading_text69(user_id))
            if text in ["🏢 Активные", "🏢 Active"]:
                rows = get_top_active()
                title = "📊 <b>Рейтинг зданий по активности</b>"
            elif text in ["💰 Цены", "💰 Prices"]:
                rows = get_top_price() if "get_top_price" in globals() else get_top_active()
                title = "💰 <b>Рейтинг зданий по средней цене</b>"
            else:
                rows = get_top_active()
                title = "📊 <b>Рейтинг ликвидности</b>"
            if not rows:
                await message.answer(no_data69(user_id, "Рейтинги"), reply_markup=main_menu(user_id))
                return
            response = title + "\n\n"
            for i, r in enumerate(rows[:10], 1):
                response += f"{i}. <b>{r.get('building_name_en') or r.get('area_name_en') or '-'}</b> — {r.get('area_name_en') or '-'} ({format_int(r.get('deals'))} сделок)\n"
            response += conclusion360_69({"deals": sum(_int(r.get('deals')) for r in rows), "avg_price": None, "avg_meter": None}, "dubai", "Dubai")
            save_report69(user_id, response, st)
            await message.answer(response, reply_markup=result_menu(user_id))
            return

        # Deals scope
        if st.get("step") == "deals_scope69":
            if text in ["🏢 По зданию", "🏢 By building"]:
                user_states[user_id] = {"step": "building_query69", "scope": "building", "direct_report": "deals"}
                await message.answer(tr(user_id, "enter_building"), reply_markup=nav_menu(user_id))
                return
            if text in ["🏙 По району", "🏙 By area"]:
                user_states[user_id] = {"step": "area_query69", "scope": "area", "direct_report": "deals"}
                await message.answer(tr(user_id, "enter_area"), reply_markup=nav_menu(user_id))
                return
            if text in ["🌆 По Дубаю", "🌆 Dubai-wide"]:
                user_states[user_id] = {"step": "choose_deal_type69", "scope": "dubai", "direct_report": "deals", "history": []}
                await ask_next_filter69(message)
                return

        # Building search input / choice
        if st.get("step") == "building_query69":
            await message.answer("🔎 Ищу здание в archive + live…")
            rows = find_buildings(text, limit=10)
            if not rows:
                await message.answer(tr(user_id, "not_found"), reply_markup=nav_menu(user_id))
                return
            suggestions = [r.get("building_name_en") for r in rows if r.get("building_name_en")]
            user_states[user_id] = {"step": "choose_building69", "scope": "building", "suggestions": suggestions, "rows": rows, "direct_report": st.get("direct_report"), "history": []}
            response = tr(user_id, "choose_building") + "\n\n"
            for i, r in enumerate(rows[:10], 1):
                response += f"{i}. <b>{r.get('building_name_en')}</b> — {r.get('area_name_en') or '-'} ({format_int(r.get('deals'))} сделок)\n"
            buttons = [[x] for x in suggestions[:10]] + [[tr(user_id, "back"), tr(user_id, "main")]]
            await message.answer(response, reply_markup=menu69(buttons))
            return

        if st.get("step") == "choose_building69":
            suggestions = st.get("suggestions") or []
            selected = None
            if text.isdigit() and 1 <= int(text) <= len(suggestions):
                selected = suggestions[int(text)-1]
            elif text in suggestions:
                selected = text
            else:
                # если пользователь написал Grande руками — берём первый найденный вариант и не перезапускаем весь сценарий
                rows = find_buildings(text, limit=1)
                if rows:
                    selected = rows[0].get("building_name_en")
            if not selected:
                await message.answer("Выберите здание кнопкой из списка или введите другое название.", reply_markup=nav_menu(user_id))
                return
            user_states[user_id] = {"step": "choose_deal_type69", "scope": "building", "name": selected, "direct_report": st.get("direct_report"), "history": []}
            await ask_next_filter69(message)
            return

        # Area search input / choice
        if st.get("step") == "area_query69":
            await message.answer("🔎 Ищу район в archive + live…")
            rows = find_areas(text, limit=10)
            if not rows:
                await message.answer(tr(user_id, "not_found"), reply_markup=nav_menu(user_id))
                return
            suggestions = [r.get("area_name_en") for r in rows if r.get("area_name_en")]
            user_states[user_id] = {"step": "choose_area69", "scope": "area", "suggestions": suggestions, "direct_report": st.get("direct_report"), "history": []}
            response = tr(user_id, "choose_area") + "\n\n"
            for i, r in enumerate(rows[:10], 1):
                response += f"{i}. <b>{r.get('area_name_en')}</b> ({format_int(r.get('deals'))} сделок)\n"
            buttons = [[x] for x in suggestions[:10]] + [[tr(user_id, "back"), tr(user_id, "main")]]
            await message.answer(response, reply_markup=menu69(buttons))
            return

        if st.get("step") == "choose_area69":
            suggestions = st.get("suggestions") or []
            selected = None
            if text.isdigit() and 1 <= int(text) <= len(suggestions):
                selected = suggestions[int(text)-1]
                ...
                rows = find_areas(text, limit=1)
                if rows:
                    selected = rows[0].get("area_name_en")
            if not selected:
                await message.answer("Выберите район кнопкой из списка или введите другое название.", reply_markup=nav_menu(user_id))
                return
            user_states[user_id] = {"step": "choose_deal_type69", "scope": "area", "name": selected, "direct_report": st.get("direct_report"), "history": []}
            await ask_next_filter69(message)
            return

        # Common filters
        if st.get("step") == "choose_deal_type69":
            if text not in [tr(user_id, "sale"), tr(user_id, "rent"), tr(user_id, "both")]:
                await ask_next_filter69(message)
                return
            st["deal_type"] = deal_type_from_button(user_id, text)
            st["step"] = "choose_property69"
            user_states[user_id] = st
            await ask_next_filter69(message)
            return

        if st.get("step") == "choose_property69":
            st["property"] = None if text == tr(user_id, "skip") else text
            st["step"] = "choose_period69"
            user_states[user_id] = st
            await ask_next_filter69(message)
            return

        if st.get("step") == "choose_period69":
            st["period"] = period_from_button(user_id, text)
            direct = st.get("direct_report")
            if direct:
                user_states[user_id] = st
                await run_report69(message, direct)
                return
            st["step"] = "choose_report69"
            user_states[user_id] = st
            await ask_next_filter69(message)
            return

        if st.get("step") == "choose_report69":
            if text in ["📊 Аналитика", "📊 Analytics"]:
                await run_report69(message, "analytics")
                return
            if text in ["🧾 Сделки", "🧾 Deals"]:
                await run_report69(message, "deals")
                return
            if text in ["📈 Периоды", "📈 Periods"]:
                await run_report69(message, "periods")
                return
            if text in ["💼 Резюме", "💼 Summary"]:
                await run_report69(message, "summary")
                return
            await ask_next_filter69(message)
            return

        # Smart selection simplified and data-backed
        if st.get("step") == "smart_goal69":
            st["goal"] = text
            st["step"] = "smart_budget69"
            user_states[user_id] = st
            await message.answer("💰 <b>Бюджет</b>\n\nВыберите диапазон:", reply_markup=menu69([["до 1M AED", "1–2M AED"], ["2–3M AED", "3–5M AED"], ["5M+ AED"], [tr(user_id, "back"), tr(user_id, "main")]]))
            return
        if st.get("step") == "smart_budget69":
            st["budget"] = text
            st["step"] = "smart_risk69"
            user_states[user_id] = st
            await message.answer("⚖️ <b>Профиль риска</b>", reply_markup=menu69([["низкий", "сбалансированный"], ["агрессивный"], [tr(user_id, "back"), tr(user_id, "main")]]))
            return
        if st.get("step") == "smart_risk69":
            await message.answer(loading_text69(user_id))
            rows = smart_pick_candidates(st.get("goal"), st.get("budget"), text, None)
            if not rows:
                await message.answer(no_data69(user_id, "Инвестиционный подбор"), reply_markup=main_menu(user_id))
                return
            best = rows[0]
            response = (
                "🧠 <b>Инвестиционный подбор</b>\n\n"
                "🏆 <b>Лучший сценарий</b>\n"
                f"📍 Район: <b>{best.get('area') or best.get('area_name_en') or '-'}</b>\n"
                f"🏠 Формат: <b>{best.get('property') or best.get('property_type') or '-'}</b>\n"
            ...
            response += conclusion360_69({"deals": best.get("deals"), "avg_price": best.get("avg_price"), "avg_meter": best.get("avg_meter")}, "area", best.get("area") or best.get("area_name_en"), best.get("property"), None, "sale")
            save_report69(user_id, response, st)
            user_states[user_id] = {"step": "done69", "scope": "area", "name": best.get("area") or best.get("area_name_en")}
            await message.answer(response, reply_markup=result_menu(user_id))
            return
        ...
        print("MAIN69_ERROR", repr(e))
        await message.answer(t69(user_id, "⚠️ Система не смогла завершить расчёт. Я сохранил сценарий — попробуйте ещё раз или расширьте фильтр.", "⚠️ Could not complete the calculation. Try again or broaden the filter."), reply_markup=main_menu(user_id))
...
# =========================
# v68 PRODUCTION DATA LAYER FIX
# =========================
# Исправляет: смешивание похожих зданий при сделках, нестабильное переключение archive/live,
# пустые выборки из-за разных схем таблиц. Все запросы ниже не используют глобальный TABLE/RЕNT_TABLE,
# а читают archive + live напрямую и объединяют результат на Python layer.

_SCHEMA_COLS_V68 = {}


def _source_url_v68(source):
    if source == "archive":
        return ARCHIVE_DATABASE_URL
    return LIVE_DATABASE_URL


def _source_tables_v68(source):
    if source == "archive":
        return ["public.dld_sale_archive", "public.dld_rent_archive"]
    return ["public.dld_transactions_full", "public.dld_rents_full"]


def _split_table_v68(table):
    if "." in table:
        a, b = table.split(".", 1)
        return a, b
    return "public", table
...
def _conn_v68(source):
    return psycopg2.connect(_source_url_v68(source), cursor_factory=RealDictCursor)


def _cols_v68(source, table):
    key = (source, table)
    if key in _SCHEMA_COLS_V68:
        return _SCHEMA_COLS_V68[key]
    schema, name = _split_table_v68(table)
    try:
        with _conn_v68(source) as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT column_name
                    FROM information_schema.columns
        ...
        print("SCHEMA_V68_ERROR", source, table, repr(e))
        cols = set()
    _SCHEMA_COLS_V68[key] = cols
    return cols


def _qcol_v68(cols, *names, default="''"):
    existing = [n for n in names if n in cols]
    if not existing:
        return default
    return "COALESCE(" + ", ".join([f"{n}::text" for n in existing]) + ", '')"


def _num_expr_v68(cols, *names):
    existing = [n for n in names if n in cols]
    if not existing:
        return "NULL::numeric"
    parts = [f"NULLIF(regexp_replace(COALESCE({n}::text, ''), '[^0-9.]', '', 'g'), '')::numeric" for n in existing]
    return "COALESCE(" + ", ".join(parts) + ")"


def _date_expr_v68(cols):
    for c in ["instance_date", "transaction_date", "contract_start_date", "contract_end_date", "created_at", "load_timestamp"]:
        if c in cols:
            return f"CASE WHEN COALESCE({c}::text,'') ~ '^\\d{{4}}-\\d{{2}}-\\d{{2}}' THEN LEFT({c}::text,10)::date ELSE NULL END"
    return "NULL::date"


def _sale_table_v68(table):
    t = table.lower()
    return "sale" in t or "transaction" in t


def _value_expr_v68(cols, table, deal_type=None):
    if is_rent_deal_type(deal_type) or (not is_sale_deal_type(deal_type) and not _sale_table_v68(table)):
        return _num_expr_v68(cols, "annual_amount", "contract_amount", "rent_value", "actual_worth")
    return _num_expr_v68(cols, "actual_worth", "procedure_amount", "sale_price", "transaction_amount")


def _meter_expr_v68(cols):
    return _num_expr_v68(cols, "meter_sale_price", "price_per_meter", "sqm_price")


def _table_allowed_for_deal_v68(table, deal_type):
    if is_sale_deal_type(deal_type):
        return _sale_table_v68(table)
    if is_rent_deal_type(deal_type):
        return not _sale_table_v68(table)
    return True


def _period_sql_v68(period):
    months = period_months(period)
    if not months:
        return "", []
    return f"AND safe_date >= CURRENT_DATE - INTERVAL '{int(months)} months'", []


def _property_sql_v68(cols, prop):
    if not prop:
        return "", []
    rooms = _qcol_v68(cols, "rooms_en", "rooms", "room_en")
    typ = _qcol_v68(cols, "property_type_en", "property_sub_type_en", "prop_type_en", "prop_sub_type_en", "property_usage_en")
    ptxt = str(prop).lower().strip()
    if ptxt == "studio":
        return f"AND (({rooms}) ILIKE %s OR ({typ}) ILIKE %s)", ["%studio%", "%studio%"]
    if ptxt in ["1 br", "2 br", "3 br", "4 br"]:
        n = ptxt.split()[0]
        return f"AND (({rooms}) ILIKE %s OR ({rooms}) ILIKE %s OR ({typ}) ILIKE %s)", [f"%{n}%", f"%{n} B/R%", f"%{n}%"]
    if ptxt == "5 br+":
        return f"AND (({rooms}) ~ %s OR ({typ}) ILIKE %s)", ["[5-9]", "%villa%"]
    mapping = {
        "villa": "%villa%", "townhouse": "%town%", "penthouse": "%penthouse%",
        "apartment": "%apartment%", "office": "%office%", "shop": "%shop%"
    }
    val = mapping.get(ptxt)
    if val:
        return f"AND ({typ}) ILIKE %s", [val]
    return "", []


def _norm_exact_v68(x):
    return normalize_search_text(x or "")


def building_aliases(name):
    # ВАЖНО: точные отчёты больше не расширяют Grande Signature -> Grande.
    # Иначе сделки по Sobha/Florea/Westwood Grande смешиваются с выбранным зданием.
    q = normalize_search_text(name)
    aliases = {
        "address opera": ["Address Residences Dubai Opera", "The Address Residences Dubai Opera"],
        "the address opera": ["Address Residences Dubai Opera", "The Address Residences Dubai Opera"],
...
def _building_condition_v68(cols, name):
    b = _qcol_v68(cols, "building_name_en", "building_en")
    p = _qcol_v68(cols, "project_name_en", "project_en", "master_project_en", "master_project")
    search = f"LOWER(TRIM(COALESCE(NULLIF({b}, ''), NULLIF({p}, ''), '')))"
    params = []
    conds = []
    for alias in building_aliases(name):
        n = _norm_exact_v68(alias)
        if n:
            conds.append(f"regexp_replace({search}, '[^a-z0-9]+', ' ', 'g') = %s")
            params.append(n)
    if not conds:
        return "AND 1=0", []
...
def _area_condition_v68(cols, name):
    area = _qcol_v68(cols, "area_name_en", "area_en")
    aliases = area_aliases(name)
    conds, params = [], []
    for a in aliases:
        n = _norm_exact_v68(a)
        if n:
            conds.append(f"regexp_replace(LOWER(TRIM({area})), '[^a-z0-9]+', ' ', 'g') = %s")
            params.append(n)
    if not conds:
        return "AND 1=0", []
    return "AND (" + " OR ".join(conds) + ")", params
...
def _iter_tables_v68(deal_type=None):
    for source in _active_sources():
        for table in _source_tables_v68(source):
            if _table_allowed_for_deal_v68(table, deal_type):
                cols = _cols_v68(source, table)
                if cols:
                    yield source, table, cols
...
def find_buildings(query, limit=10):
    ...
    tokens = smart_query_tokens(query)
    bucket = {}
    for source, table, cols in _iter_tables_v68(None):
        b_expr = _qcol_v68(cols, "building_name_en", "building_en")
        p_expr = _qcol_v68(cols, "project_name_en", "project_en", "master_project_en", "master_project")
        area_expr = _qcol_v68(cols, "area_name_en", "area_en")
        name_expr = f"COALESCE(NULLIF({b_expr}, ''), NULLIF({p_expr}, ''), '')"
        hay_expr = f"LOWER(COALESCE({b_expr}, '') || ' ' || COALESCE({p_expr}, '') || ' ' || COALESCE({area_expr}, ''))"
        where = [f"{name_expr} <> ''"]
        params = []
        for t in tokens:
            where.append(f"{hay_expr} ILIKE %s")
            params.append(f"%{t}%")
        try:
            with _conn_v68(source) as conn:
                with conn.cursor() as cur:
                    cur.execute(f"""
                        SELECT {name_expr} AS building_name_en, {area_expr} AS area_name_en, COUNT(*) AS deals
                        FROM {table}
                        WHERE {' AND '.join(where)}
                        GROUP BY {name_expr}, {area_expr}
                        ORDER BY COUNT(*) DESC
                        LIMIT %s
                    """, params + [limit])
                    for r in cur.fetchall():
                        key = (_norm_exact_v68(r.get("building_name_en")), _norm_exact_v68(r.get("area_name_en")))
                        if not key[0]:
                            continue
                        item = bucket.setdefault(key, {"building_name_en": r.get("building_name_en"), "area_name_en": r.get("area_name_en"), "deals": 0})
                        item["deals"] += _int(r.get("deals"))
        except Exception as e:
            print("FIND_BUILDINGS_V68_ERROR", source, table, repr(e))
    return sorted(bucket.values(), key=lambda x: _int(x.get("deals")), reverse=True)[:limit]
...
def find_areas(query, limit=10):
    ...
    for source, table, cols in _iter_tables_v68(None):
        area_expr = _qcol_v68(cols, "area_name_en", "area_en")
        b_expr = _qcol_v68(cols, "building_name_en", "project_name_en", "master_project_en")
        where = [f"{area_expr} <> ''"]
        params = []
        for t in tokens:
            where.append(f"LOWER({area_expr}) ILIKE %s")
            params.append(f"%{t}%")
        try:
            with _conn_v68(source) as conn:
                with conn.cursor() as cur:
                    cur.execute(f"""
                        SELECT {area_expr} AS area_name_en, COUNT(*) AS deals, COUNT(DISTINCT {b_expr}) AS buildings
                        FROM {table}
                        WHERE {' AND '.join(where)}
                        GROUP BY {area_expr}
                        ORDER BY COUNT(*) DESC
                    ...
                    """, params + [limit])
                    for r in cur.fetchall():
                        key = _norm_exact_v68(r.get("area_name_en"))
                        item = bucket.setdefault(key, {"area_name_en": r.get("area_name_en"), "deals": 0, "buildings": 0})
                        item["deals"] += _int(r.get("deals"))
                        item["buildings"] += _int(r.get("buildings"))
        except Exception as e:
            print("FIND_AREAS_V68_ERROR", source, table, repr(e))
    return sorted(bucket.values(), key=lambda x: _int(x.get("deals")), reverse=True)[:limit]


def _aggregate_stats_rows_v68(rows):
    if not rows:
        return None
    deals = sum(_int(r.get("deals")) for r in rows)
    if deals <= 0:
        return None
    def wavg(field):
        vals = [(float(r[field]), _int(r.get("deals"))) for r in rows if r.get(field) is not None and _int(r.get("deals")) > 0]
        if not vals:
            return None
        return sum(v*w for v,w in vals) / sum(w for _,w in vals)
    return {
        "deals": deals,
        "avg_price": wavg("avg_price"),
        "min_price": min([float(r["min_price"]) for r in rows if r.get("min_price") is not None], default=None),
        "max_price": max([float(r["max_price"]) for r in rows if r.get("max_price") is not None], default=None),
        "avg_meter": wavg("avg_meter"),
        "buildings": sum(_int(r.get("buildings")) for r in rows),
        "first_date": min([r.get("first_date") for r in rows if r.get("first_date")], default=None),
        "last_date": max([r.get("last_date") for r in rows if r.get("last_date")], default=None),
    }


def get_stats(scope="dubai", name=None, prop=None, period=None, deal_type=None):
    partial = []
    for source, table, cols in _iter_tables_v68(deal_type):
        date_expr = _date_expr_v68(cols)
        val_expr = _value_expr_v68(cols, table, deal_type)
        meter_expr = _meter_expr_v68(cols)
        building_expr = _qcol_v68(cols, "building_name_en", "project_name_en", "master_project_en")
        where = [f"{val_expr} IS NOT NULL", f"{val_expr} > 0"]
        params = []
        if scope == "building" and name:
            c, a = _building_condition_v68(cols, name); where.append(c.replace("AND ", "", 1)); params += a
        elif scope == "area" and name:
            c, a = _area_condition_v68(cols, name); where.append(c.replace("AND ", "", 1)); params += a
        psql, pargs = _property_sql_v68(cols, prop); 
        if psql: where.append(psql.replace("AND ", "", 1)); params += pargs
        per_sql, _ = _period_sql_v68(period)
                ...
                with conn.cursor() as cur:
                    cur.execute(f"""
                        SELECT COUNT(*) AS deals,
                               ...
                               MAX(safe_date) AS last_date
                        FROM (
                            SELECT {date_expr} AS safe_date, {val_expr} AS price, {meter_expr} AS meter_price, {building_expr} AS building_name
                            FROM {table}
                            WHERE {inner_where}
                        ) x
                        WHERE 1=1 {outer}
                    """, params)
                    r = cur.fetchone()
                    if r and _int(r.get("deals")) > 0:
                        partial.append(r)
        except Exception as e:
            print("GET_STATS_V68_ERROR", source, table, repr(e))
    return _aggregate_stats_rows_v68(partial)
...
def get_latest_deals(scope, name, prop=None, period=None, deal_type=None, limit=5, unit_query=None):
    ...
    for source, table, cols in _iter_tables_v68(deal_type):
        ...
        b_expr = _qcol_v68(cols, "building_name_en", "project_name_en", "master_project_en")
        area_expr = _qcol_v68(cols, "area_name_en", "area_en")
        rooms_expr = _qcol_v68(cols, "rooms_en", "rooms")
        typ_expr = _qcol_v68(cols, "property_sub_type_en", "property_type_en", "prop_sub_type_en", "prop_type_en", "property_usage_en")
        proc_expr = _qcol_v68(cols, "procedure_name_en", "procedure_name", "reg_type_en", "property_usage_en", "tenant_type_en")
        where = [f"{val_expr} IS NOT NULL", f"{val_expr} > 0"]
        params = []
        if scope == "building" and name:
            c, a = _building_condition_v68(cols, name); where.append(c.replace("AND ", "", 1)); params += a
        ...
        try:
            with _conn_v68(source) as conn:
                with conn.cursor() as cur:
                    cur.execute(f"""
                        SELECT * FROM (
                            SELECT {date_expr} AS safe_date,
                                   {proc_expr} AS procedure_name_en,
                                   {rooms_expr} AS rooms_en,
                                   {typ_expr} AS property_type_en,
                                   {typ_expr} AS property_sub_type_en,
                                   {val_expr} AS price,
                                   {meter_expr} AS meter_price,
                                   {b_expr} AS building_name_en,
            ...
            print("GET_LATEST_DEALS_V68_ERROR", source, table, repr(e))
    # Дополнительная защита: для здания оставляем только точное нормализованное имя.
    if scope == "building" and name:
        allowed = {_norm_exact_v68(a) for a in building_aliases(name)}
        rows = [r for r in rows if _norm_exact_v68(r.get("building_name_en")) in allowed]
    rows.sort(key=lambda r: (r.get("safe_date") is not None, r.get("safe_date")), reverse=True)
    return rows[:limit]


def get_latest_deals_smart(scope, name, prop=None, period=None, deal_type=None, limit=5, unit_query=None):
    # Для выбранного здания/района НЕ расширяем scope и НЕ смешиваем похожие здания.
    attempts = [
        (prop, period, deal_type),
        (prop, None, deal_type),
        (None, period, deal_type),
...
def get_top_active():
    ...
    for source, table, cols in _iter_tables_v68(None):
        b_expr = _qcol_v68(cols, "building_name_en", "project_name_en", "master_project_en")
        area_expr = _qcol_v68(cols, "area_name_en", "area_en")
        val_expr = _value_expr_v68(cols, table, None)
        meter_expr = _meter_expr_v68(cols)
        try:
            with _conn_v68(source) as conn:
                with conn.cursor() as cur:
                    cur.execute(f"""
                        SELECT {b_expr} AS building_name_en, {area_expr} AS area_name_en,
                               COUNT(*) AS deals, AVG({val_expr}) AS avg_price, AVG({meter_expr}) AS avg_meter
                        FROM {table}
                        WHERE {b_expr} <> '' AND {val_expr} IS NOT NULL AND {val_expr} > 0
                        GROUP BY {b_expr}, {area_expr}
                        ORDER BY COUNT(*) DESC LIMIT 20
                    """)
                    for r in cur.fetchall():
                        key = (_norm_exact_v68(r.get("building_name_en")), _norm_exact_v68(r.get("area_name_en")))
                        item = bucket.setdefault(key, {"building_name_en": r.get("building_name_en"), "area_name_en": r.get("area_name_en"), "deals": 0, "avg_price": None, "avg_meter": None})
                        item["deals"] += _int(r.get("deals"))
                        item["avg_price"] = r.get("avg_price") or item.get("avg_price")
                        item["avg_meter"] = r.get("avg_meter") or item.get("avg_meter")
        except Exception as e:
            print("GET_TOP_ACTIVE_V68_ERROR", source, table, repr(e))
    return sorted(bucket.values(), key=lambda x: _int(x.get("deals")), reverse=True)[:10]


def get_top_buildings_in_scope(scope="dubai", name=None, period=None, deal_type=None, limit=7):
    if scope == "dubai":
        return get_top_active()[:limit]
    bucket = {}
    for source, table, cols in _iter_tables_v68(deal_type):
        b_expr = _qcol_v68(cols, "building_name_en", "project_name_en", "master_project_en")
        area_expr = _qcol_v68(cols, "area_name_en", "area_en")
        ...
        meter_expr = _meter_expr_v68(cols)
        date_expr = _date_expr_v68(cols)
        where = [f"{b_expr} <> ''", f"{val_expr} IS NOT NULL", f"{val_expr} > 0"]
        params=[]
        ...
        try:
            with _conn_v68(source) as conn:
                with conn.cursor() as cur:
                    cur.execute(f"""
                        SELECT building_name_en, area_name_en, COUNT(*) AS deals, AVG(price) AS avg_price, AVG(meter_price) AS avg_meter
                        FROM (SELECT {b_expr} AS building_name_en, {area_expr} AS area_name_en, {val_expr} AS price, {meter_expr} AS meter_price, {date_expr} AS safe_date FROM {table} WHERE {' AND '.join(where)}) x
                        WHERE 1=1 {per_sql}
                        GROUP BY building_name_en, area_name_en ORDER BY COUNT(*) DESC LIMIT %s
                    """, params+[limit])
                    for r in cur.fetchall():
                        key=(_norm_exact_v68(r.get("building_name_en")), _norm_exact_v68(r.get("area_name_en")))
                        item=bucket.setdefault(key,{"building_name_en":r.get("building_name_en"),"area_name_en":r.get("area_name_en"),"deals":0,"avg_price":None,"avg_meter":None})
                        item["deals"] += _int(r.get("deals")); item["avg_price"] = r.get("avg_price") or item.get("avg_price"); item["avg_meter"] = r.get("avg_meter") or item.get("avg_meter")
        except Exception as e:
            print("GET_TOP_BUILDINGS_SCOPE_V68_ERROR", source, table, repr(e))
    return sorted(bucket.values(), key=lambda x: _int(x.get("deals")), reverse=True)[:limit]

print("Loaded v68 production data layer: exact building scope + archive/live direct aggregation")
