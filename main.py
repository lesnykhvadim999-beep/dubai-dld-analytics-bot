        ...
        with conn.cursor() as cur:
            cur.execute("""
                SELECT
                    ...
                    AVG(NULLIF(actual_worth, '')::numeric) AS avg_price,
                    AVG(NULLIF(meter_sale_price, '')::numeric) AS avg_meter_price
                FROM public.dld_transactions_full
                WHERE NULLIF(actual_worth, '') IS NOT NULL
            """)
            return cur.fetchone()
        ...
        with conn.cursor() as cur:
            cur.execute("""
                ...
                WHERE building_name_en IS NOT NULL
                  ...
                  AND NULLIF(actual_worth, '') IS NOT NULL
