...
# =========================
# RATING FIX v74 — robust archive/live rankings only
# =========================
# This block is intentionally isolated: it fixes only the Ratings section.

RATING_ENTITY_BUILDINGS = {"🏢 Здания", "🏢 Buildings", "🏢 المباني"}
RATING_ENTITY_AREAS = {"🏙 Районы", "🏙 Areas", "🏙 المناطق"}
RATING_METRIC_DEALS = {"📊 По сделкам", "📊 By deals", "📊 حسب الصفقات", tr(0, "top_active") if False else ""}
RATING_METRIC_PRICE = {"💰 По цене", "💰 By price", "💰 حسب السعر", tr(0, "top_price") if False else ""}
RATING_METRIC_GROWTH = {"📈 По росту", "📈 Growth", "📈 النمو"}
RATING_METRIC_LIQUIDITY = {"💧 Ликвидность", "💧 Liquidity", "💧 السيولة"}
RATING_MENU_TEXTS = {"📊 Рейтинги", "📊 Рейтинг", "📊 Rankings", "📊 Rating", "📊 التصنيفات"}


def rating_menu(user_id):
    """Compact rating menu; does not affect other flows."""
    return kb([
        ["🏢 Здания", "🏙 Районы"],
        ["📊 По сделкам", "💰 По цене"],
...
def _rating_table_name(source):
    return "public.dld_sale_archive" if source == "archive" else "public.dld_transactions_full"


def _rating_connect(source):
    return psycopg2.connect(_rating_pg_url(source), cursor_factory=RealDictCursor)


def _rating_columns(conn, table_name):
    schema, table = table_name.split(".", 1)
    with conn.cursor() as cur:
        cur.execute("""
            SELECT column_name
...
def _rating_first_text(cols, names, fallback="''"):
    available = [f"NULLIF({n}::text, '')" for n in names if n in cols]
    return "COALESCE(" + ", ".join(available + [fallback]) + ")" if available else fallback


def _rating_num_expr(cols, names):
    parts = []
    for n in names:
        if n in cols:
            parts.append(f"NULLIF(regexp_replace(COALESCE({n}::text, ''), '[^0-9.]', '', 'g'), '')::numeric")
    if not parts:
        return "NULL::numeric"
    return "COALESCE(" + ", ".join(parts) + ")"


def _rating_date_expr(cols):
    for n in ["instance_date", "transaction_date", "date", "created_at", "load_timestamp"]:
        if n in cols:
            return f"CASE WHEN {n}::text ~ '^\\d{{4}}-\\d{{2}}-\\d{{2}}' THEN ({n}::text)::date ELSE NULL END"
    return "NULL::date"


def _rating_source_rows(source, entity="building", metric="deals", limit=20):
    table = _rating_table_name(source)
    try:
        with _rating_connect(source) as conn:
            cols = _rating_columns(conn, table)
            if not cols:
                return []

            building_expr = _rating_first_text(cols, [
                "building_name_en", "building_en", "project_name_en", "project_en", "master_project_en"
            ])
            area_expr = _rating_first_text(cols, [
                "area_name_en", "area_en", "area_name", "procedure_area", "master_project_en"
            ])
            price_expr = _rating_num_expr(cols, [
                "actual_worth", "actual_worth_norm", "trans_value", "transaction_value", "price", "sale_price"
            ])
            meter_expr = _rating_num_expr(cols, [
                "meter_sale_price", "meter_sale_price_norm", "price_per_sqft", "meter_price"
            ])
            date_expr = _rating_date_expr(cols)

            name_expr = building_expr if entity == "building" else area_expr
            area_select = area_expr if entity == "building" else "''::text"

            if metric == "growth" and date_expr != "NULL::date":
                sql = f"""
                    SELECT
                        {name_expr} AS name,
                        {area_select} AS area_name_en,
                        COUNT(*) AS deals,
                        AVG({price_expr}) AS avg_price,
                        AVG({meter_expr}) AS avg_meter,
                        AVG({price_expr}) FILTER (WHERE {date_expr} >= CURRENT_DATE - INTERVAL '12 months') AS cur_avg_price,
                        AVG({price_expr}) FILTER (WHERE {date_expr} < CURRENT_DATE - INTERVAL '12 months'
                                                 AND {date_expr} >= CURRENT_DATE - INTERVAL '24 months') AS prev_avg_price,
                        COUNT(*) FILTER (WHERE {date_expr} >= CURRENT_DATE - INTERVAL '12 months') AS cur_deals
                    FROM {table}
                    WHERE COALESCE({name_expr}, '') <> ''
                    GROUP BY {name_expr}, {area_select}
                    HAVING COUNT(*) FILTER (WHERE {date_expr} >= CURRENT_DATE - INTERVAL '12 months') >= 2
                    ORDER BY
                        CASE
                            WHEN AVG({price_expr}) FILTER (WHERE {date_expr} < CURRENT_DATE - INTERVAL '12 months'
                                                           AND {date_expr} >= CURRENT_DATE - INTERVAL '24 months') > 0
                            THEN (
                                (AVG({price_expr}) FILTER (WHERE {date_expr} >= CURRENT_DATE - INTERVAL '12 months')
                                - AVG({price_expr}) FILTER (WHERE {date_expr} < CURRENT_DATE - INTERVAL '12 months'
                                                            AND {date_expr} >= CURRENT_DATE - INTERVAL '24 months'))
                                / AVG({price_expr}) FILTER (WHERE {date_expr} < CURRENT_DATE - INTERVAL '12 months'
                                                            AND {date_expr} >= CURRENT_DATE - INTERVAL '24 months')
                            )
                            ELSE 0
                ...
                sql = f"""
                    SELECT
                        ...
                        AVG({price_expr}) AS avg_price,
                        AVG({meter_expr}) AS avg_meter,
                        NULL::numeric AS cur_avg_price,
                        NULL::numeric AS prev_avg_price,
                        COUNT(*) AS cur_deals
                    FROM {table}
                    WHERE COALESCE({name_expr}, '') <> ''
                    ...
                    GROUP BY {name_expr}, {area_select}
                    HAVING COUNT(*) >= 1
                    ORDER BY AVG({price_expr}) DESC NULLS LAST, COUNT(*) DESC
                    LIMIT %s
                """
            else:
                # deals / liquidity: do not require price. This is the key fix.
                sql = f"""
                    SELECT
                        {name_expr} AS name,
                        {area_select} AS area_name_en,
        ...
        print(f"RATING_SOURCE_{source.upper()}_ERROR:", repr(e))
        return []


def _rating_merge(rows, entity="building", metric="deals", limit=10):
    grouped = {}
    for r in rows:
        name = str(r.get("name") or r.get("building_name_en") or r.get("area_name_en") or "").strip()
        if not name:
            continue
        area = str(r.get("area_name_en") or "").strip()
        key = (name.lower(), area.lower() if entity == "building" else "")
        g = grouped.setdefault(key, {"name": name, "area_name_en": area, "deals": 0, "_price_sum": 0.0, "_price_w": 0, "_meter_sum": 0.0, "_meter_w": 0, "growth_pct": None})
        deals = _int(r.get("deals"))
        g["deals"] += deals
        avg_price = _num(r.get("avg_price"))
        avg_meter = _num(r.get("avg_meter"))
        if avg_price and deals:
            g["_price_sum"] += avg_price * deals
            g["_price_w"] += deals
        if avg_meter and deals:
            g["_meter_sum"] += avg_meter * deals
            g["_meter_w"] += deals
        cur = _num(r.get("cur_avg_price"))
        prev = _num(r.get("prev_avg_price"))
        if cur and prev and prev > 0:
            gp = ((cur - prev) / prev) * 100
            if g["growth_pct"] is None or gp > g["growth_pct"]:
                g["growth_pct"] = gp
    out = []
    for g in grouped.values():
        g["avg_price"] = g["_price_sum"] / g["_price_w"] if g["_price_w"] else None
        g["avg_meter"] = g["_meter_sum"] / g["_meter_w"] if g["_meter_w"] else None
        g.pop("_price_sum", None); g.pop("_price_w", None); g.pop("_meter_sum", None); g.pop("_meter_w", None)
        out.append(g)
    if metric == "price":
        out.sort(key=lambda x: _num(x.get("avg_price")) or 0, reverse=True)
    elif metric == "growth":
        out.sort(key=lambda x: _num(x.get("growth_pct")) or -999999, reverse=True)
    else:
        out.sort(key=lambda x: _int(x.get("deals")), reverse=True)
    return out[:limit]


def get_rating_rows(entity="building", metric="deals", limit=10):
    rows = []
    ...
    for source in sources:
        rows.extend(_rating_source_rows(source, entity=entity, metric=metric, limit=max(limit * 3, 20)))
    return _rating_merge(rows, entity=entity, metric=metric, limit=limit)


def get_top_active():
    rows = get_rating_rows(entity="building", metric="deals", limit=10)
    return [{"building_name_en": r["name"], "area_name_en": r.get("area_name_en"), "deals": r.get("deals"), "avg_price": r.get("avg_price"), "avg_meter": r.get("avg_meter")} for r in rows]


def get_top_price():
    rows = get_rating_rows(entity="building", metric="price", limit=10)
    return [{"building_name_en": r["name"], "area_name_en": r.get("area_name_en"), "deals": r.get("deals"), "avg_price": r.get("avg_price"), "avg_meter": r.get("avg_meter")} for r in rows]


def format_rating_report(user_id, entity="building", metric="deals"):
    rows = get_rating_rows(entity=entity, metric=metric, limit=10)
    if not rows:
        return no_data_message("Рейтинг")
    entity_title = "зданий" if entity == "building" else "районов"
    metric_title = {
        "deals": "по количеству сделок",
        "price": "по средней цене",
        "growth": "по росту цены",
        "liquidity": "по ликвидности",
    }.get(metric, "по сделкам")
    text = f"📊 <b>Рейтинг {entity_title} {metric_title}</b>\n\n"
    for i, r in enumerate(rows, 1):
        name = r.get("name")
        area = r.get("area_name_en")
        text += f"{i}. <b>{name}</b>"
        if entity == "building" and area:
            text += f" — {area}"
        text += f"\n   📊 Сделок: <b>{format_int(r.get('deals') or 0)}</b>"
        if r.get("avg_price"):
            text += f"\n   💰 Средняя цена: <b>{format_money(r.get('avg_price'))}</b>"
        if r.get("avg_meter"):
            text += f"\n   📐 Цена за м²: <b>{format_money(r.get('avg_meter'))}</b>"
        if metric == "growth" and r.get("growth_pct") is not None:
            text += f"\n   📈 Рост: <b>{r.get('growth_pct'):.1f}%</b>"
        text += "\n\n"
    text += "🧠 <b>Экономическое заключение 360°</b>\n\n"
    if metric in ("deals", "liquidity"):
        text += "Высокое количество сделок означает лучшую ликвидность: объект или район легче оценивать, покупать и перепродавать."
    elif metric == "price":
        text += "Высокая средняя цена показывает премиальный сегмент рынка. Для инвестора важно дополнительно проверить доходность и глубину спроса."
    else:
        text += "Рост цены показывает положительную динамику, но перед покупкой важно сверить его с ликвидностью и количеством сделок."
    return text
...
async def main_handler(message: Message):
    ...
    try:
        ...
        if text in RATING_MENU_TEXTS:
            push_state(user_id, {"step": "rating_menu", "rating_entity": "building"})
            await message.answer("📊 <b>Рейтинги DLD</b>\n\nВыберите, что показать:", reply_markup=rating_menu(user_id))
            return

        if text in RATING_ENTITY_BUILDINGS:
            state["step"] = "rating_menu"
            state["rating_entity"] = "building"
            user_states[user_id] = state
            await message.answer("🏢 <b>Рейтинг зданий</b>\n\nВыберите метрику:", reply_markup=rating_menu(user_id))
            return

        if text in RATING_ENTITY_AREAS:
            state["step"] = "rating_menu"
            state["rating_entity"] = "area"
            user_states[user_id] = state
            await message.answer("🏙 <b>Рейтинг районов</b>\n\nВыберите метрику:", reply_markup=rating_menu(user_id))
            return

        if text in RATING_METRIC_DEALS or text == tr(user_id, "top_active"):
            entity = state.get("rating_entity", "building")
            await message.answer("⏳ Формирую рейтинг по сделкам DLD...")
            await message.answer(format_rating_report(user_id, entity=entity, metric="deals"), reply_markup=rating_menu(user_id))
            return

        if text in RATING_METRIC_PRICE or text == tr(user_id, "top_price"):
            entity = state.get("rating_entity", "building")
            await message.answer("⏳ Формирую рейтинг по средней цене DLD...")
            await message.answer(format_rating_report(user_id, entity=entity, metric="price"), reply_markup=rating_menu(user_id))
            return

        if text in RATING_METRIC_GROWTH:
            entity = state.get("rating_entity", "building")
            await message.answer("⏳ Считаю динамику цен по DLD...")
            await message.answer(format_rating_report(user_id, entity=entity, metric="growth"), reply_markup=rating_menu(user_id))
            return

        if text in RATING_METRIC_LIQUIDITY:
            entity = state.get("rating_entity", "building")
            await message.answer("⏳ Считаю ликвидность по DLD...")
            await message.answer(format_rating_report(user_id, entity=entity, metric="liquidity"), reply_markup=rating_menu(user_id))
            return
