    ...
    try:
        ...
        if state.get("step") == "choose_building":
            ...
            if not chosen:
                ...
                await start_building_search_from_text(message, text)
                return
            stored_name = chosen.get("name") or text
            if chosen.get("area"):
                stored_name = f"{stored_name}|||{chosen.get('area')}"
            user_states[user_id] = {"step": "choose_deal_type", "scope": "building", "name": stored_name, "force_report": state.get("force_report"), "history": state.get("history", [])}
            await message.answer(tr(user_id, "choose_deal_type"), reply_markup=deal_type_menu(user_id))
            return
