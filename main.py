...
def sale_meta_v40():
    ...
    search = " || ' ' || ".join([
        text40(cols, ["building_en", "building_name_en", "building_name", "project_en", "project_name_en", "master_project_en"]),
        text40(cols, ["area_en", "area_name_en", "area_name", "area"]),
        text40(cols, ["transaction_number", "transaction_id", "procedure_name", "procedure_name_en"]),
    ])
    return {
        "table": SALE_TABLE_V40,
        "date": date40(cols, ["transaction_date", "instance_date", "created_at", "load_timestamp"]),
        "area": text40(cols, ["area_name_en", "area_en", "area_name", "area", "community", "district"]),
        "building": text40(cols, ["building_name_en", "building_en", "building_name", "project_en", "project_name_en", "master_project_en", "project_name"]),
        "project": text40(cols, ["project_en", "project_name_en", "master_project_en", "project_name", "building_en", "building_name_en"]),
        "rooms": text40(cols, ["rooms_en", "rooms", "bedrooms", "room", "rooms_count"]),
        "ptype": text40(cols, ["property_type_en", "prop_type_en", "property_type", "prop_type", "usage_en"]),
        "subtype": text40(cols, ["property_sub_type_en", "prop_sub_type_en", "prop_sb_type_en", "property_subtype", "unit_type"]),
...
def rent_meta_v40():
    ...
    search = " || ' ' || ".join([
        text40(cols, ["building_name_en", "building_en", "project_en", "project_name_en", "master_project_en", "project_name"]),
        text40(cols, ["area_name_en", "area_en", "area_name", "area", "community", "district"]),
    ...
    return {
        ...
        "area": text40(cols, ["area_name_en", "area_en", "area_name", "area", "community", "district"]),
        "building": text40(cols, ["building_name_en", "building_en", "project_en", "project_name_en", "master_project_en", "project_name"]),
        "project": text40(cols, ["project_en", "project_name_en", "master_project_en", "project_name", "building_en", "building_name_en"]),
        "rooms": text40(cols, ["rooms_en", "rooms", "bedrooms", "room", "rooms_count"]),
        "ptype": text40(cols, ["property_type_en", "prop_type_en", "property_type", "prop_type", "usage_en", "property_usage_en"]),
        "subtype": text40(cols, ["property_sub_type_en", "prop_sub_type_en", "property_subtype", "unit_type"]),
        "procedure": "'Rent'",
        "unit": text40(cols, ["unit_number", "unit_no", "unit", "property_number", "property_no", "unit_id"]),
        "price": price,
        "meter": meter_expr,
...
def _scope_condition_v40(scope="dubai", name=None, original_query=None):
    ...
    if scope == "area":
        ...
        return "AND (" + " OR ".join(["area_name_en ILIKE %s OR search_text ILIKE %s" for _ in vals]) + ")", [x for v in vals for x in (f"%{v}%", f"%{v.lower()}%")]
    if scope == "building":
...
def _property_condition_v40(prop):
    if not prop:
        return "", []
    p = str(prop).lower().strip()
    blob = "LOWER(COALESCE(rooms_en,'') || ' ' || COALESCE(property_type_en,'') || ' ' || COALESCE(property_sub_type_en,''))"
    if p == "studio":
    ...
    try:
        with db() as conn:
            with conn.cursor() as cur:
                cur.execute(f"""
                    WITH unified AS (
                        SELECT building_name_en, area_name_en, value_price, search_text
                        {base_from_v40('sale')}
                        UNION ALL
                        SELECT building_name_en, area_name_en, value_price, search_text
                        {base_from_v40('rent')}
                    ...
                    SELECT
                        COALESCE(building_name_en, '') AS building_name_en,
                        COALESCE(area_name_en, '') AS area_name_en,
                        COUNT(*) AS deals
                    FROM unified
                    WHERE COALESCE(building_name_en, '') <> ''
                      AND {token_sql}
                    GROUP BY COALESCE(building_name_en, ''), COALESCE(area_name_en, '')
                    ORDER BY deals DESC
                    LIMIT %s
                """, params + [limit])
                return cur.fetchall()
...
def find_areas(query, limit=10):
    ...
    cond = " OR ".join(["area_name_en ILIKE %s OR search_text ILIKE %s" for _ in vals])
    ...
    try:
        with db() as conn:
            with conn.cursor() as cur:
                cur.execute(f"""
                    WITH unified AS (
                        SELECT area_name_en, building_name_en, search_text
                        {base_from_v40('sale')}
                        UNION ALL
                        SELECT area_name_en, building_name_en, search_text
                        {base_from_v40('rent')}
                    ...
                    SELECT
                        ...
                        COUNT(DISTINCT NULLIF(building_name_en, '')) AS buildings
                    FROM unified
                    WHERE COALESCE(area_name_en, '') <> ''
                      AND ({cond})
                    GROUP BY COALESCE(area_name_en, '')
                    ORDER BY deals DESC
                    LIMIT %s
                """, params + [limit])
                return cur.fetchall()
...
def get_top_active():
    try:
        with db() as conn:
            with conn.cursor() as cur:
                cur.execute(f"""
                    SELECT building_name_en, area_name_en, COUNT(*) AS deals, AVG(value_price) AS avg_price, AVG(value_meter) AS avg_meter
                    {base_from_v40('sale')}
                    WHERE COALESCE(building_name_en, '') <> '' AND value_price IS NOT NULL AND value_price > 0
