...
async def main_handler(message: Message):
    ...
    try:
        ...
        if state.get("step") == "building_query":
            ...
            # Важно: кнопка должна хранить не только building, но и area.
            # Иначе Grande смешивается с Sobha Grande / Florea Grande / Westwood Grande.
            building_options = {}
            button_labels = []
            for r in rows[:10]:
                b = (r.get("building_name_en") or "").strip()
                a = (r.get("area_name_en") or "").strip()
                if not b:
                    continue
                label = f"{b} — {a}" if a else b
                # Telegram-кнопка может быть длинной, но точность важнее: выбранный объект должен быть уникальным.
                building_options[label] = {"building": b, "area": a}
                button_labels.append(label)

            new_state = {
                "step": "choose_building",
                "scope": "building",
                "suggestions": button_labels,
                "building_options": building_options,
                "history": state.get("history", [])
            }
            user_states[user_id] = new_state

            buttons = [[name] for name in button_labels]
            buttons.append([tr(user_id, "back"), tr(user_id, "main")])
        ...
        if state.get("step") == "choose_building":
            ...
            if text not in state.get("suggestions", []):
                if looks_like_free_search(text) and not is_navigation_text(user_id, text):
                    await start_building_search_from_text(message, text)
                    return
                await message.answer("Выберите здание кнопкой из списка — так бот точно не смешает разные здания с похожим названием.")
                return

            selected = options.get(text, {})
            state["name"] = selected.get("building") or text
            state["selected_area"] = selected.get("area")
            state["selected_label"] = text
        ...
        if state.get("step") == "choose_period":
            ...
            if state.get("force_report") == "last":
                # После выбранных фильтров сразу показываем сделки.
                # Больше не спрашиваем повторно здание/район и не просим unit number,
                # потому что в DLD у нас пока нет стабильного поля номера юнита.
                state["step"] = "choose_report"
        ...
        if state.get("step") == "choose_report":
            ...
            display_name = state.get("selected_label") or state.get("name")
            name = state.get("name")
            if scope == "building" and state.get("selected_area"):
                name = f"{name}|||{state.get('selected_area')}"
            prop = state.get("property")
            period = state.get("period")
            deal_type = state.get("deal_type")

            if text in [tr(user_id, "full_report"), "📊 Аналитика", "📊 Полная аналитика", "Полная аналитика"]:
                await message.answer(tr(user_id, "loading"))
                row, used_prop, used_period, used_deal_type = get_stats_smart(scope, name, prop, period, deal_type)
                title = "🌆 <b>Статистика Дубая</b>"
                if scope == "building":
                    title = f"🏢 <b>{display_name}</b>"
                elif scope == "area":
                    title = f"🏙 <b>{display_name}</b>"
            ...
            if text == "💼 Экономическое резюме":
                if scope == "dubai":
                    ...
                    return
                title = f"🏢 <b>{display_name}</b>" if scope == "building" else f"🏙 <b>{name}</b>"
                await message.answer(tr(user_id, "loading"))
                row = get_unit_summary(scope, name, prop, period, deal_type)
            ...
            if text in [tr(user_id, "last_deals"), "🧾 Сделки", "📊 По сделкам", "Сделки", "Последние сделки"]:
                await message.answer(tr(user_id, "loading"))
...
# =========================
# FLOW + EXACT SELECTION FIX v70
# =========================
# Исправляет два критичных сценария:
# 1) выбранное здание хранится как building|||area, чтобы Grande не смешивался с другими Grande;
# 2) аренда и продажа корректно понимают этот формат в archive + live источниках.

def _split_building_area_v70(name):
    raw = clean_query(str(name or ""))
    if "|||" in raw:
        b, a = raw.split("|||", 1)
        return clean_query(b), clean_query(a)
    # поддержка старых кнопок вида "Building — Area"
    for sep in [" — ", " | ", " - "]:
        if sep in raw:
            b, a = raw.split(sep, 1)
            return clean_query(b), clean_query(a)
...
def scope_condition(scope="dubai", name=None, original_query=None):
    ...
    if scope == "area":
        return make_area_exact_condition(original_query or name)
    if scope == "building":
        building, area = _split_building_area_v70(original_query or name)
        b_sql, b_params = make_building_condition(building)
        if area:
            return b_sql + " AND LOWER(COALESCE(area_name_en::text, '')) ILIKE %s", b_params + [f"%{area.lower()}%"]
        return b_sql, b_params
    return "", []


_ORIG_rent_scope_condition_v34_v70 = rent_scope_condition_v34 if 'rent_scope_condition_v34' in globals() else None

def rent_scope_condition_v34(scope, name, strict=True):
    if not strict or not name:
        return '', []
    n, selected_area = _split_building_area_v70(name)
    if not n:
        return '', []
    if scope == 'building':
        parts = ["LOWER(COALESCE(building_name_en::text,'')) ILIKE %s", "search_text ILIKE %s"]
        ...
        if selected_area:
            parts.append("LOWER(COALESCE(area_name_en::text,'')) ILIKE %s")
            params.append(f'%{selected_area.lower()}%')
        return 'AND (' + ' OR '.join(parts) + ')', params
    if scope == 'area':
        values = area_alias_values(n) or [n]
        return 'AND (' + ' OR '.join(["LOWER(COALESCE(area_name_en::text,'')) ILIKE %s OR search_text ILIKE %s" for _ in values]) + ')', sum([[f'%{v.lower()}%', f'%{v.lower()}%'] for v in values], [])
    return '', []


print("Loaded flow + exact building selection fix v70")
